...
export default function Home() {
    ...
    // Keep document fields introduced by the other compatible implementation.
    ...
    const last = useMemo(() => Object.entries(progress).filter(([, item]) => {
    ...
    const lastPoint = useMemo(() => {
        ...
        const latest = Object.entries(lastState.episodes).sort((a, b) => b[1].updatedAt - a[1].updatedAt)[0];
        if (!latest) return {season: lastState.season ?? 1, episode: lastState.episode, state: undefined};
        const separator = latest[0].indexOf(":");
        return {
            season: Number(latest[0].slice(0, separator)) || lastState.season || 1,
            episode: latest[0].slice(separator + 1) || lastState.episode,
            state: latest[1],
        };
    }, [lastState]);
        ...
        const activeTracker = tracked.find(t => t.animeId === active.anime_id || t.animeIds?.includes(active.anime_id)),
            goCatalog = () => {
                ...
                setTimeout(() => document.getElementById("catalog")?.scrollIntoView({behavior: "smooth"}), 0);
                                     ...
                                     onExport={exportConfig} onImport={importConfig}/>;
                      ...
                      onProgress={(v, originEpisodeKey, changedEpisodeKey) => {
                          ...
                          setProgress(current => {
                              const newlyWatched = keys.some(key => !isEpisodeWatched(current[active.anime_id]?.episodes[key]) && isEpisodeWatched(v.episodes[key])),
                                  next = {...current, [active.anime_id]: v};
                              write(K.progress, next);
    ...
    return <main className="app"><Header query={query} setQuery={setQuery} onSearch={() => {
        ...
        {view === "home" && <>{partyPresence.session && partyHostPlayback && <section className="party-host-now">
            ...
            <section
                ...
                <div className="hero-widgets">
                    ...
                    <div className="hero-box alerts"><h3>Отслеживаю <span>{tracked.length}</span></h3>
                        <div className="hero-scroll hero-tracking-list">{tracked.map(t => {
                            ...
                            return <article className="hero-track-card" key={t.animeId}>
                                ...
                                }}><b>{t.title}</b><ReleaseMark anime={anime}
                                ...
                                <div className="hero-track-actions">{t.newEpisodes > 0 &&
                                    ...
                                    <em className={t.newEpisodes > 0 ? "release-status new" : "release-status quiet"}><i/>{t.newEpisodes > 0 ? `Новая серия · +${t.newEpisodes}` : "Новых серий нет"}
                                    </em></div>
                            </article>;
        ...
        <footer><Brand/><span className="api-thanks">Огромная благодарность разработчикам YummyAnime за предоставленный API — только благодаря им был создан AnimeSoul.</span><span>Прогресс и настройки сохраняются на этом ПК</span><span
            className="app-version">Версия {packageInfo.displayVersion ?? packageInfo.version}</span></footer>
        {folderPicker &&
...
function FAQBlock() {
    return <details className="catalog-guide">
        ...
        <div className="catalog-guide-content">
            ...
            <article><b>Перенос файлом из папки проекта</b><p>Закройте AnimeSoul. Скопируйте
                файл <code>data/animesoul-storage.json</code> из старой папки проекта в папку <code>data</code> новой
                версии с заменой существующего файла, затем снова запустите сайт батником. Для другого устройства
                перенесите этот файл любым удобным способом.</p></article>
            ...
            <article><b>Приватность</b><p>Аккаунт не обязателен. Пользовательские данные находятся локально
                в <code>data/animesoul-storage.json</code>; браузерное хранилище используется как резервное зеркало.</p>
            </article>
