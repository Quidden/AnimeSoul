...
export function migrateDocument(input: Partial<StorageDocument> | null | undefined): StorageDocument {
  ...
  const activeProfile = ensured.some((profile) => profile.id === input?.activeProfile)
    ...
    : ensured[0].id;
  return {
    // Preserve fields introduced by another compatible AnimeSoul build.
    // This keeps save transfers forward-compatible in both directions.
    ...input,
...
const currentSitePort =
  typeof window !== "undefined" ? Number(window.location.port || "3001") : 3001;
export const STORAGE_URL =
  typeof window !== "undefined"
    ? `${window.location.protocol}//${window.location.hostname}:${currentSitePort + 1}/storage`
    : "http://127.0.0.1:3002/storage";
