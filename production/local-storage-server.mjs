    ...
    try {
      ...
      if (request.method === "POST" && url.pathname === "/watch-party/create") {
        ...
        rooms.set(id, {
          id,
          hostToken: token,
          roomMode: body.roomMode === "shared" ? "shared" : "host",
          createdAt: Date.now(),
          updatedAt: Date.now(),
          playback: null,
          actionSeq: 0,
          participants: new Map([[token, {
            id: token,
            name: String(body.name || "Хост").slice(0, 32),
            role: "host",
            mode: "follow",
            updatedAt: Date.now(),
          }]]),
      ...
      if (request.method === "POST" && url.pathname === "/watch-party/transfer-host") {
        ...
        room.updatedAt = Date.now();
        return reply(response, 200, { ok: true, hostId: nextHost.id });
      }
      ...
      if (request.method === "POST" && url.pathname === "/watch-party/leave") {
        const body = await readBody(request), room = rooms.get(String(body.roomId || "").toUpperCase());
        if (room && body.token === room.hostToken) {
          room.participants.delete(body.token);
          const nextHost = [...room.participants.values()].sort((a, b) => b.updatedAt - a.updatedAt)[0];
          if (nextHost) {
            nextHost.role = "host";
            room.hostToken = nextHost.id;
          } else {
