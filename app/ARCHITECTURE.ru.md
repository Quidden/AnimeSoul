...
[`docs/PROJECT_MAP.md`](docs/PROJECT_MAP.ru.md).
...
восемь router-модулей на desktop (Watch Party исключён на Android) и при наличии production bundle добавляет static/SPA
fallback.

| Router | Контракт | Делегирует |
| --- | --- | --- |
| `api/yummy.py` | `/api/yummy` | `HybridCatalogueService` → Yummy + Kodik |
...
[`docs/API_REFERENCE.md`](docs/API_REFERENCE.ru.md).
...
профиль. Backend требует непустой массив profiles, непустые строковые ID и object snapshots; заданный activeProfile обязан существовать. Остальные доменные поля остаются непрозрачным JSON. Миграция известных полей выполняется
...
[`docs/DATA_MODEL.md`](docs/DATA_MODEL.ru.md).
...
-> useProfileAutosave (debounce 400 ms)
...
-> HybridCatalogueService -> YummyAnimeGateway / KodikAnimeGateway
...
[`docs/ENTRY_POINTS_AND_FLOWS.md`](docs/ENTRY_POINTS_AND_FLOWS.ru.md).
...
[`docs/STYLES.md`](docs/STYLES.ru.md).
...
## Дополнительные границы текущей реализации

- `api/kodik.py`: direct stream и ping; `api/downloads.py`: очередь, файлы, settings; `api/episode_dates.py`: даты Jikan.
- `catalog.py` объединяет источники и хранит реестр идентичности; Kodik resolver отделяет подпись от загрузок.
- `fieldUpdatedAt` сохраняет независимые версии настроек/коллекций, `resetAt` предотвращает возврат сброшенного прогресса. Профили при merge объединяются по ID.
- `choice_pending` защищён на backend; cloud restore создаёт backup; storage locks общие для одного пути.
- Видео/постеры/iframe могут загружаться клиентом напрямую; API каталога и файловые операции проходят через backend.
- Android мосты связывают UI с MediaStore/MediaSession/PiP/Cast; очередь download и комнаты не переживают перезапуск процесса.

Подробные владельцы и связи: [UI](docs/UI.ru.md), [Backend](docs/BACKEND.ru.md), [API](docs/API_REFERENCE.ru.md).
