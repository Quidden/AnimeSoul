...
приватного ключей Kodik в разделе ключей настроек; приватный ключ хранится
...
[`docs/PLATFORMS.md`](docs/PLATFORMS.ru.md).
...
Начните с [`docs/README.md`](docs/README.ru.md). Там собраны ссылки на:
...
прочитайте [`SAVE_COMPATIBILITY.md`](SAVE_COMPATIBILITY.ru.md) и
...
[UI](docs/UI.ru.md), [Backend](docs/BACKEND.ru.md), [46 маршрутов и все модели запросов](docs/API_SCHEMA.ru.md), [галерея](docs/SCREENSHOTS.ru.md). Из app: `python tools/check_docs.py` проверяет пары языков, ссылки и таблицу API; после изменения маршрутов выполните `python tools/check_docs.py --write-api`.

Первый sync защищён на сервере; snapshot хранит fieldUpdatedAt, сброс прогресса — resetAt. См. обновлённые [данные](docs/DATA_MODEL.ru.md) и [Drive](docs/GDRIVE_SYNC.ru.md). Язык документации не переключает русский UI приложения.
