...
export default function Home() {
    ...
    useEffect(() => {
        ...
        let pending = 0, failed = false, settleTimer: ReturnType<typeof setTimeout> | undefined;
        const report = (status: ApiStatus) => {
            ...
            settleTimer = setTimeout(() => {
                if (pending > 0) return;
                const status: ApiStatus = failed ? {state: "error", at: Date.now()} : {
                    state: "updated",
                    at: Date.now()
                };
                failed = false;
        ...
        const wrapped: typeof window.fetch = async (input, init) => {
            const url = typeof input === "string" ? input : input instanceof URL ? input.href : input.url,
                isYummy = url.includes("/api/yummy") && !url.includes("silent=1");
            if (!isYummy) return originalFetch(input, init);
            clearTimeout(settleTimer);
            pending++;
    ...
    useEffect(() => {
        const loadedFavorites = read<number[]>(K.favorites, []), loadedFolders = read<Folder[]>(K.folders, []),
            loadedProgress = read<Progress>(K.progress, {}), loadedTracked = read<Tracker[]>(K.tracked, []),
            loadedTheme = read<Theme>(K.theme, THEMES[0]), loadedToolbar = read<ToolbarPosition>(K.toolbar, "bottom"),
            loadedHistoryClearedAt = read<number>(K.historyClearedAt, 0),
            loadedHistoryEnabled = read<boolean>(K.historyEnabled, true),
            active = localStorage.getItem(K.activeProfile) ?? "default";
    ...
    useEffect(() => {
        ...
        document.documentElement.dataset.colorScheme = isLightColor(theme.background) ? "light" : "dark";
        document.documentElement.style.colorScheme = isLightColor(theme.background) ? "light" : "dark";
    ...
    const load = async (next = 0, append = false, q = query) => {
        ...
        setError("");
        try {
            const r = await fetch(`/api/yummy?mode=catalog&limit=24&offset=${next}&q=${encodeURIComponent(q)}`),
                p = await r.json();
    ...
    const loadMore = async () => {
        ...
        setError("");
        try {
            const existingIds = new Set(catalog.map(a => a.anime_id)), matches = (a: Anime) => {
                const familyCount = a.franchiseCount ?? 1, isMovie = isMovieAnime(a);
                return (genre === "Все" || a.genres?.some(g => g.title === genre)) && (!yearFrom || (a.year ?? 0) >= +yearFrom) && (!yearTo || (a.year ?? 9999) <= +yearTo) && (groupFilter === "all" || (groupFilter === "franchise" ? familyCount > 1 : familyCount === 1)) && (formatFilter === "all" || (formatFilter === "movie" ? isMovie : !isMovie));
            }, before = new Set(groupFranchises(catalog).filter(matches).map(a => franchiseKey(a.title)));
            let cursor = offset + 24, fresh: Anime[] = [], addedCards = 0;
            for (let attempt = 0; attempt < 5 && addedCards < 12; attempt++) {
                const r = await fetch(`/api/yummy?mode=catalog&limit=48&offset=${cursor}`), p = await r.json();
                if (!r.ok) throw Error(p.error);
                const page = (p.anime ?? []) as Anime[];
                for (const anime of page) if (!existingIds.has(anime.anime_id)) {
                    existingIds.add(anime.anime_id);
                    fresh.push(anime);
                }
                cursor += 48;
                addedCards = groupFranchises([...catalog, ...fresh]).filter(matches).filter(a => !before.has(franchiseKey(a.title))).length;
                if (page.length < 48) break;
                if (addedCards < 12) await new Promise(resolve => setTimeout(resolve, 120));
            }
            setCatalog(current => [...new Map([...current, ...fresh].map(a => [a.anime_id, a])).values()]);
            setOffset(cursor - 24);
            if (!fresh.length) setError("Больше новых аниме в каталоге не найдено");
        } catch (e) {
            setError(e instanceof Error ? e.message : "Не удалось загрузить новые аниме");
        } finally {
            setLoading(false);
        }
    };
    useEffect(() => {
        if (view === "catalog" && !active && !catalogReady) void load(0, false, "");
    ...
    useEffect(() => {
        ...
        fetch(`/api/yummy?mode=details&ids=${missing.join(",")}`).then(r => r.json()).then(p => setCatalog(current => [...current, ...((p.anime ?? []) as Anime[]).filter(a => !current.some(x => x.anime_id === a.anime_id))])).catch(() => {
        });
        ...
        (async () => {
                ...
                try {
                    ...
                    if (r.ok) {
                        const videos = p.videos as Video[],
                            unique = [...new Map(videos.map(v => [v.number, v])).values()];
                        rows.push([id, unique.length, unique.reduce((sum, v) => sum + (v.duration ?? 0), 0)]);
                    }
                } catch {
                }
            ...
            setProgress(current => {
                ...
                for (const [id, total, totalDuration] of rows) next[id] = {
                    ...next[id],
                    title: next[id]?.title ?? known(id)?.title,
    ...
    const saveFav = (v: number[]) => {
        setFavorites(v);
        write(K.favorites, v);
    }, saveFolders = (v: Folder[]) => {
        setFolders(v);
        write(K.folders, v);
    }, saveProgress = (v: Progress) => {
        setProgress(v);
        write(K.progress, v);
    }, saveTracked = (v: Tracker[]) => {
        setTracked(v);
        write(K.tracked, v);
    };
    ...
    const toggleFolder = (folder: Folder, id: number) => saveFolders(folders.map(f => f.id === folder.id ? {
        ...f,
    ...
    const visible = useMemo(() => franchises.filter(a => {
        const meta = cardMeta[a.anime_id], familyCount = meta?.familyCount ?? a.franchiseCount ?? 1,
            isMovie = isMovieAnime(a);
        return (genre === "Все" || a.genres?.some(g => g.title === genre)) && (!yearFrom || (a.year ?? 0) >= +yearFrom) && (!yearTo || (a.year ?? 9999) <= +yearTo) && (groupFilter === "all" || !meta || (groupFilter === "franchise" ? familyCount > 1 : familyCount === 1)) && (formatFilter === "all" || (formatFilter === "movie" ? isMovie : !isMovie));
    }).sort((a, b) => sort === "rating-desc" ? (b.rating?.average ?? 0) - (a.rating?.average ?? 0) : sort === "rating-asc" ? (a.rating?.average ?? 0) - (b.rating?.average ?? 0) : sort === "year-desc" ? (b.year ?? 0) - (a.year ?? 0) : sort === "year-asc" ? (a.year ?? 0) - (b.year ?? 0) : (b.views ?? 0) - (a.views ?? 0)), [franchises, genre, yearFrom, yearTo, sort, groupFilter, formatFilter, cardMeta]);
    ...
    useEffect(() => {
        if (active) return;
        const allowed = view === "catalog" ? franchises : franchises.filter(anime => storedIds.includes(anime.anime_id)),
            targets = allowed.filter(a => !cardMeta[a.anime_id]);
        ...
        (async () => {
            for (const anime of targets) {
                if (cancelled) break;
                try {
                    const root = franchiseName(anime.title), family = await fetchFamily(anime, root),
                        members = family.length ? family : [anime], movies = members.filter(isMovieAnime),
                        series = members.filter(a => !isMovieAnime(a) && !isExtraAnime(a)),
                        arcCount = new Set(series.map(a => stripPart(a.title))).size,
                        numberedSeasons = new Set(series.map(a => a.season).filter((n): n is number => typeof n === "number")).size,
                        seasonCount = Math.max(arcCount, numberedSeasons, series.length > 1 ? 2 : series.length),
                        movieCount = movies.length,
                        statusSource = members.find(a => releaseStatus(a).kind === "airing") ?? members.find(a => releaseStatus(a).kind === "planned") ?? anime,
                        status = releaseStatus(statusSource),
                        videoResponse = await fetch(`/api/yummy?mode=videos&id=${anime.anime_id}`),
                        videoPayload = await videoResponse.json(), videos = (videoPayload.videos ?? []) as Video[],
                        unique = [...new Map(videos.map(v => [v.number, v])).values()],
                        durations = unique.map(v => v.duration ?? 0).filter(Boolean), meta: CardMeta = {
                    ...
                    if (!cancelled) setCardMeta(current => ({...current, ...shared, [anime.anime_id]: meta}));
                } catch {
                }
    ...
    }, [franchises.map(a => a.anime_id).join(","), storedIds.join(","), view, active]);
    ...
    useEffect(() => {
        if (!lastAnime || !playerPrefs.homeEpisodePreview) {
            setHeroPreviewAnime(null);
            return;
        }
        let cancelled = false;
        fetch(`/api/yummy?mode=details&ids=${lastState?.originAnimeId ?? lastAnime.anime_id}`).then(r => r.json()).then(payload => {
            if (!cancelled) setHeroPreviewAnime((payload.anime?.[0] as Anime | undefined) ?? lastAnime);
        }).catch(() => {
            if (!cancelled) setHeroPreviewAnime(lastAnime);
        });
        return () => {
            cancelled = true;
        };
    }, [lastAnime?.anime_id, lastState?.originAnimeId, playerPrefs.homeEpisodePreview]);
    useEffect(() => {
        ...
        fetch(`/api/yummy?mode=videos&id=${lastState.originAnimeId ?? lastAnime.anime_id}`)
            ...
            .catch(() => {
                if (!cancelled) setHeroPreviewVideo(null);
            });
        return () => {
            cancelled = true;
        };
    ...
    const historyItems = useMemo(() => historyEnabled ? Object.entries(progress).flatMap(([animeId, item]) => Object.entries(item.episodes).filter(([, state]) => state.updatedAt > historyClearedAt).map(([key, state]) => {
        const [season, episode] = key.split(":");
        return {animeId: +animeId, season: +season || 1, episode, state};
    })).sort((a, b) => b.state.updatedAt - a.state.updatedAt).slice(0, 30) : [], [progress, historyClearedAt, historyEnabled]);
    const watchingItems = useMemo(() => Object.entries(progress).flatMap(([animeId, item]) => {
    ...
    const animeProgress = (p?: AnimeProgress) => {
    ...
    const statistics = useMemo<AnimeStatistics>(() => {
        ...
        for (const [idText, item] of Object.entries(progress)) {
            const animeId = +idText, anime = known(animeId), episodeEntries = Object.entries(item.episodes),
                states = episodeEntries.map(([, state]) => state), completed = states.filter(isEpisodeWatched),
                completionCount = states.reduce((sum, state) => sum + (state.completions ?? (isEpisodeWatched(state) ? 1 : 0)), 0),
                rewatchCount = Math.max(0, completionCount - completed.length);
            ...
            if (rewatchCount > 0) rewatches.push({
                animeId,
            ...
            for (const [episodeKey, state] of episodeEntries) {
                ...
                const separator = episodeKey.indexOf(":"),
                    season = Number(episodeKey.slice(0, separator)) || item.season || 1,
                    episode = separator >= 0 ? episodeKey.slice(separator + 1) : episodeKey;
                ...
                for (const timestamp of timestamps) activity.push({
                    timestamp,
                    animeId,
                    title: anime?.title ?? `Аниме #${animeId}`,
                    season,
        ...
        return migrateSnapshot(
            {
                ...
                favorites,
                folders,
                progress: Object.fromEntries(Object.entries(progress).map(([animeId, item]) => [animeId, {
                    ...item,
                    title: item.title ?? known(Number(animeId))?.title ?? previous?.animeTitles?.[Number(animeId)],
                }])),
                animeTitles: {
    ...
    const exportConfig = () => {
        ...
        const blob = new Blob([JSON.stringify(makeSnapshot(profileName), null, 2)], {type: "application/json"}),
    ...
    const changeHistoryEnabled = (enabled: boolean) => {
        const now = Date.now();
    ...
    if (active) {
        const activeTracker = tracked.find(t => t.animeId === active.anime_id || t.animeIds?.includes(active.anime_id)),
            goCatalog = () => {
                ...
                setTimeout(() => document.getElementById("catalog")?.scrollIntoView({behavior: "smooth"}), 0);
            }, watchHeader = <Header query={query} setQuery={setQuery} onSearch={searchCatalog} onCatalog={goCatalog}
                                     onLibrary={openLibrary} theme={theme} setTheme={setTheme} playerPrefs={playerPrefs}
                                     setPlayerPrefs={setPlayerPrefs} historyEnabled={historyEnabled}
                                     ...
                                     onSuggestion={a => {
                                         ...
                                         setResumeRequested(false);
                                         setNewEpisodeRequested(false);
                                     }} profiles={profiles} activeProfile={activeProfile} onSwitchProfile={switchProfile}
                                     onExport={exportConfig} onImport={importConfig} onStorageReload={reloadStorage}/>;
        return <Watch header={watchHeader} anime={active} resumeRequested={resumeRequested}
                      newEpisodeRequested={newEpisodeRequested} favorite={favorites.includes(active.anime_id)}
                      onFavorite={() => toggleFavorite(active.anime_id)} onBack={goCatalog} onLibrary={openLibrary}
                      ...
                      onProgress={(v, originEpisodeKey, changedEpisodeKey) => {
                          const keys = Array.isArray(changedEpisodeKey) ? changedEpisodeKey : [changedEpisodeKey ?? `${v.season ?? 1}:${v.episode}`],
                              originKeys = Array.isArray(originEpisodeKey) ? originEpisodeKey : originEpisodeKey ? [originEpisodeKey] : [];
                          setProgress(current => {
                              const newlyWatched = keys.some(key => !isEpisodeWatched(current[active.anime_id]?.episodes[key]) && isEpisodeWatched(v.episodes[key])),
                                  next = {...current, [active.anime_id]: {
                                      ...v,
                                      // Human-readable metadata only. Progress is still keyed by anime ID.
                                      title: active.title,
                                  }};
                              write(K.progress, next);
                              if (activeTracker && originKeys.length && newlyWatched) setTracked(currentTracked => {
                                  const nextTracked = currentTracked.map(t => t.animeId === activeTracker.animeId ? originKeys.reduce((updated, key) => acknowledgeTrackedEpisode(updated, key), t) : t);
                                  write(K.tracked, nextTracked);
                                  return nextTracked;
                              });
                              return next;
                          });
                      ...
                      onFolders={() => setFolderPicker(active)} tracker={activeTracker}
                      onTrack={(count, dubs, animeIds, title, knownKeys) => saveTracked([...tracked.filter(t => t.animeId !== activeTracker?.animeId), {
                      ...
                      folderPicker={folderPicker} folders={folders} toggleFolder={toggleFolder}
                      createFolder={createFolder} closePicker={() => setFolderPicker(null)}/>;
    }
    return <main className="app"><Header query={query} setQuery={setQuery} onSearch={() => {
        setView("catalog");
        void load(0, false, query);
        window.scrollTo({top: 0, behavior: "smooth"});
    }} onCatalog={() => {
        ...
        window.scrollTo({top: 0, behavior: "smooth"});
                                         ...
                                         onSwitchProfile={switchProfile} onExport={exportConfig}
                                         onImport={importConfig} onStorageReload={reloadStorage}/>
        {view === "home" && <>{partyPresence.session && partyHostPlayback && <section className="party-host-now">
            <span><i/><small>СОВМЕСТНЫЙ ПРОСМОТР · {partyPresence.party?.roomMode === "shared" ? "ОБЩЕЕ УПРАВЛЕНИЕ" : "УПРАВЛЯЕТ ХОСТ"}</small><b>{partyHost?.name ?? "Хост"} смотрит: {partyHostAnime?.title ?? `аниме #${partyHostPlayback.animeId}`}</b><em>Сезон {partyHostPlayback.season} · серия {partyHostPlayback.episode} · {formatTime(partyHostPlayback.position)}</em></span>{partyHostAnime &&
            <button className="primary" onClick={() => openAnime(partyHostAnime, false)}>Открыть просмотр</button>}
        </section>}
            <section
                className={`hero dashboard-hero ${lastAnime && lastState && playerPrefs.homeEpisodePreview ? "with-episode-preview" : ""}`}>
                <div className="hero-content"><span
                    className="eyebrow">{lastAnime && lastState ? "ПРОДОЛЖИТЬ ПРОСМОТР" : "ВЫБРАТЬ ЧТО ПОСМОТРЕТЬ"}</span>{lastAnime && lastState ? <>
                    <h1>{lastAnime.title}</h1>
                    <p>{lastState.seasonLabel ?? `Сезон ${lastPoint?.season ?? lastState.season ?? 1}`} ·
                        серия {lastDisplayEpisode} · остановились
                        на {formatTime(lastPoint?.state?.position ?? 0)}</p>{playerPrefs.homeEpisodePreview ?
                    <EpisodeSlideshow key={playerPrefs.homePreviewMode} className="hero-episode-preview"
                                      images={playerPrefs.homePreviewMode === "screenshots" ? episodePreviewImages(heroPreviewAnime ?? lastAnime, lastDisplayEpisode) : []}
                                      fallback={heroPreviewAnime?.poster?.fullsize ?? heroPreviewAnime?.poster?.big ?? lastAnime.poster?.fullsize ?? lastAnime.poster?.big}
                                      allowLowQuality={playerPrefs.homePreviewMode === "screenshots"}
                                      label={`${lastState.seasonLabel ?? `Сезон ${lastPoint?.season ?? lastState.season ?? 1}`} · Серия ${lastDisplayEpisode}`}
                                      sublabel={`Продолжить с ${formatTime(lastPoint?.state?.position ?? 0)}`}
                                      iframeUrl={playerPrefs.homePreviewMode === "screenshots" ? heroPreviewVideo?.iframe_url : undefined}
                                      duration={heroPreviewVideo?.duration ?? lastPoint?.state?.duration ?? 1440}
                                      onClick={() => openAnime(lastAnime, true)}/> :
                    <button className="primary" onClick={() => openAnime(lastAnime, true)}>▶ Продолжить
                        просмотр</button>}</> : <><h1>Твоя коллекция.<br/><i>Твои правила.</i></h1><p>Сейчас нет
                    незавершённого просмотра — выбери новое аниме в каталоге.</p>
                    ...
                    <details className="compact-options hero-options">
                        ...
                        <div><Toggle label="Автоматически запускать продолжение" value={playerPrefs.autoPlayResume}
                                     onChange={v => {
                                         const next = {...playerPrefs, autoPlayResume: v};
                                         setPlayerPrefs(next);
                                         write(K.playerPrefs, next);
                                     }}/><Toggle label="Предпросмотр серии на главной"
                                                 value={playerPrefs.homeEpisodePreview} onChange={v => {
                            const next = {...playerPrefs, homeEpisodePreview: v};
                            setPlayerPrefs(next);
                            write(K.playerPrefs, next);
                        }}/>
                            <div
                                className={`preview-mode-field ${playerPrefs.homeEpisodePreview ? "enabled" : "disabled"}`}>
                                <span>Изображение предпросмотра</span>
                                <div className="preview-mode-switch" role="group"
                                     aria-label="Изображение предпросмотра">
                                    <button type="button" disabled={!playerPrefs.homeEpisodePreview}
                                            className={playerPrefs.homePreviewMode === "poster" ? "active" : ""}
                                            onClick={() => {
                                                const next = {...playerPrefs, homePreviewMode: "poster" as const};
                                                setPlayerPrefs(next);
                                                write(K.playerPrefs, next);
                                            }}>HD-картинка
                                    </button>
                                    <button type="button" disabled={!playerPrefs.homeEpisodePreview}
                                            ...
                                            onClick={() => {
                                                const next = {...playerPrefs, homePreviewMode: "screenshots" as const};
                                                setPlayerPrefs(next);
                                                write(K.playerPrefs, next);
                                            }}>Кадры серии
                                    </button>
                                </div>
                                {playerPrefs.homeEpisodePreview && playerPrefs.homePreviewMode === "screenshots" &&
                                    <small>Загружается качественный беззвучный видеопредпросмотр серии.</small>}</div>
                        </div>
                    </details>
                </div>
                <div className="hero-widgets">
                    <div className="hero-box"><h3>♥ Избранное <span>{favorites.length}</span></h3>
                        <div className="mini-list hero-scroll">{favorites.map(id => {
                            const anime = known(id);
                            return <div className="hero-mini-row" key={id}>
                                <button className="hero-mini-main" onClick={() => anime && setActive(anime)}>
                                    <span>{anime?.title ?? `Аниме #${id}`}<ReleaseMark anime={anime}
                                                                                       status={cardMeta[id]?.status}/></span><small>{animeProgress(progress[id])}%</small>
                                </button>
                                <button className="hero-mini-delete" title="Удалить из избранного"
                                        aria-label={`Удалить ${anime?.title ?? `аниме #${id}`} из избранного`}
                                        onClick={() => saveFav(favorites.filter(item => item !== id))}>
                                    <svg viewBox="0 0 24 24" aria-hidden="true">
                                        <path
                                            d="M9 3h6l1 2h4v2H4V5h4l1-2Zm-3 6h12l-1 12H7L6 9Zm3 2v7h2v-7H9Zm4 0v7h2v-7h-2Z"/>
                                    </svg>
                                </button>
                            </div>;
                        })}</div>
                    ...
                    <div className="hero-box">
                        <h3>Папки <span className="hero-folder-actions">{lastDeletedFolder &&
                            <button title={`Восстановить папку «${lastDeletedFolder.folder.name}»`}
                                    onClick={restoreLastFolder}>↶</button>}
                            <button title="Новая папка" onClick={createFolder}>＋</button></span></h3>
                        <div className="hero-scroll">{folders.map(f => {
                            const s = folderStats(f);
                            return <div className="hero-mini-row hero-folder-row" key={f.id}>
                                <button className="folder-progress" onClick={() => setOpenedFolder(f)}>
                                    <span>{f.name}<small>{s.watched}/{s.total} серий</small></span><i><b
                                    style={{width: `${s.percent}%`}}/></i><em>{s.percent}%</em></button>
                                <button className="hero-mini-delete" title={`Удалить папку «${f.name}»`}
                                        aria-label={`Удалить папку ${f.name}`} onClick={() => deleteFolder(f)}>
                                    <svg viewBox="0 0 24 24" aria-hidden="true">
                                        <path
                                            d="M9 3h6l1 2h4v2H4V5h4l1-2Zm-3 6h12l-1 12H7L6 9Zm3 2v7h2v-7H9Zm4 0v7h2v-7h-2Z"/>
                                    </svg>
                    ...
                    <div className="hero-box alerts"><h3>Отслеживаю <span>{tracked.length}{totalNewEpisodes > 0 && <em className="track-total-new">+{totalNewEpisodes}</em>}</span></h3>
                        <div className="hero-scroll hero-tracking-list">{sortedTracked.map(t => {
                            const anime = known(t.animeId);
                            return <article className="hero-track-card" key={t.animeId}>
                                <button className="hero-track-main" onClick={() => {
                                    if (anime) {
                                        setNewEpisodeRequested(false);
                                        setActive(anime);
                                    }
                                }}><b>{t.title}{t.newEpisodes > 0 && <em className="track-ep-badge">+{t.newEpisodes}</em>}{(t.otherDubEpisodes ?? 0) > 0 && <em className="track-ep-badge other-dub" title="Новая серия уже есть, но пока только в другой озвучке">другая озвучка · +{t.otherDubEpisodes}</em>}</b><ReleaseMark anime={anime}
                                                                status={cardMeta[t.animeId]?.status}/><small>{t.dubs?.length ? `Озвучки: ${t.dubs.join(", ")}` : "Все озвучки"}</small><span>{t.knownEpisodes} известных серий</span>
                                </button>
                                <div className="hero-track-actions">{t.newEpisodes > 0 &&
                                    <button className="watch-new-button" onClick={() => {
                                        if (anime) {
                                            setResumeRequested(false);
                                            setNewEpisodeRequested(true);
                                            setActive(anime);
                                        }
                                    }}>▶ Смотреть новую серию</button>}
                                    <button className="untrack-button"
                                            onClick={() => saveTracked(tracked.filter(x => x.animeId !== t.animeId))}>Отписаться
                                    </button>
                                    {t.newEpisodes > 0 && <em className="release-status new"><i/>Новая серия · +{t.newEpisodes}</em>}
                                    {(t.otherDubEpisodes ?? 0) > 0 && <em className="release-status other-dub" title="Серия доступна в другой озвучке, но ещё не появилась в отслеживаемой"><i/>Есть в другой озвучке · +{t.otherDubEpisodes}</em>}
                                    {t.newEpisodes <= 0 && (t.otherDubEpisodes ?? 0) <= 0 && <em className="release-status quiet"><i/>Новых серий нет</em>}
                                </div>
                            </article>;
                        })}{!tracked.length && <small>Подписок пока нет</small>}</div>
                    </div>
                </div>
            </section>
            <section className="library" id="my-library">
                <div className="section-collapse-control">
                    <button className={`collapse-toggle ${libraryExpanded ? "expanded" : ""}`}
                            aria-expanded={libraryExpanded} onClick={() => {
                        const next = !libraryExpanded;
                        write(K.libraryExpanded, next);
                        setLibraryExpanded(next);
                    }}><i>⌄</i><span>Папки и избранное</span></button>
                </div>
                <div className={`collapse-shell ${libraryExpanded ? "expanded" : ""}`}>
                    <div className="collapse-inner">
                        <div className="section-inline-actions">{lastDeletedFolder &&
                            <button className="outline restore-folder" onClick={restoreLastFolder}>↶ Восстановить
                                «{lastDeletedFolder.folder.name}»</button>}
                            <button className="outline" onClick={createFolder}>＋ Новая папка</button>
                        </div>
                        <div className="collection-grid">
                            <div className="collection-card favorites-card"><h3>♥ Избранное</h3>
                                <b>{favorites.length} тайтлов
                                    · {favoriteStats.watched} из {favoriteStats.total} серий</b>
                                <div className="wide-progress"><i style={{width: `${favoriteStats.percent}%`}}/></div>
                                <div>{favorites.map(id => {
                                    const anime = known(id);
                                    return <div className="collection-row" draggable key={id}
                                                onDragStart={e => e.dataTransfer.setData("text/plain", String(id))}
                                                onDragOver={e => e.preventDefault()} onDrop={e => {
                                        e.preventDefault();
                                        const from = Number(e.dataTransfer.getData("text/plain"));
                                        saveFav(reorder(favorites, from, id));
                                    }}><i className="drag">⠿</i>
                                        <button className="collection-title" onClick={() => anime && setActive(anime)}>
                                            <b>{anime?.title ?? `Аниме #${id}`}<ReleaseMark anime={anime}
                                                                                            status={cardMeta[id]?.status}/></b><span>{animeProgress(progress[id])}%</span>
                                        </button>
                                        <button className="collection-remove" title="Удалить из избранного"
                                                onClick={() => saveFav(favorites.filter(item => item !== id))}>×
                                        </button>
                                    </div>;
                                })}</div>
                            ...
                            {folders.map(f => {
                                const s = folderStats(f);
                                return <div className="collection-card" key={f.id} role="button" tabIndex={0}
                                            onClick={() => setOpenedFolder(f)}><h3>{f.name}</h3>
                                    <b>{f.animeIds.length} тайтлов · {s.watched} из {s.total} серий</b>
                                    <div className="wide-progress"><i style={{width: `${s.percent}%`}}/></div>
                                    <div>{f.animeIds.map(id => {
                                        const anime = known(id);
                                        return <div className="collection-row compact" key={id}>
                                            <button
                                                className="collection-title">{anime?.title ?? `Аниме #${id}`}<ReleaseMark
                                                anime={anime} status={cardMeta[id]?.status}/></button>
                                            <button className="collection-remove" title="Удалить из папки"
                                                    onClick={event => {
                ...
                {watchingItems.length > 0 && <section className="watching-section">
                    <div className="section-collapse-control">
                        <button className={`collapse-toggle ${watchingExpanded ? "expanded" : ""}`}
                                aria-expanded={watchingExpanded} onClick={() => {
                            const next = !watchingExpanded;
                            write(K.watchingExpanded, next);
                            setWatchingExpanded(next);
                        }}><i>⌄</i><span>Смотрю сейчас</span></button>
                    </div>
                    <div className={`collapse-shell ${watchingExpanded ? "expanded" : ""}`}>
                        <div className="collapse-inner">
                            <div className="section-inline-actions"><span>{watchingItems.length} тайтлов</span></div>
                            <div className="watching-list">{watchingItems.map(entry => {
                                const anime = known(entry.animeId), whole = watchTimeProgress(entry.item);
                                return <article key={entry.animeId}>{anime?.poster?.big &&
                                    <img src={anime.poster.big} alt=""/>}
                                    <button className="watching-main" onClick={() => anime && openAnime(anime, true)}>
                                        <b>{anime?.title ?? `Аниме #${entry.animeId}`}</b><ReleaseMark anime={anime}
                                                                                                       status={cardMeta[entry.animeId]?.status}/><small>Сезон {entry.season} ·
                                        серия {entry.episode} · {formatTime(entry.state.position)}</small>
                                        <div className="wide-progress"><i style={{width: `${whole}%`}}/></div>
                                        <span>{whole}% просмотрено</span></button>
                                    <div className="watching-actions">{anime &&
                                        <button className="primary" onClick={() => openAnime(anime, true)}>▶
                                            Продолжить</button>}
                                        <button className="watching-remove" title="Убрать из списка «Смотрю сейчас»"
                                                onClick={() => {
                ...
                <section className="history-collapsible">
                    <div className="section-collapse-control history-collapse-control">
                        <button className={`collapse-toggle ${historyExpanded ? "expanded" : ""}`}
                                aria-expanded={historyExpanded} onClick={() => {
                            const next = !historyExpanded;
                            write(K.historyExpanded, next);
                            setHistoryExpanded(next);
                        }}>
                            <i>⌄</i><span>История просмотра</span><small>{historyEnabled ? "Просмотры сохраняются" : "История выключена"}</small>
                        </button>
                        <div className="history-enable-control" onClick={event => event.stopPropagation()}><Toggle
                            label="Сохранять историю" value={historyEnabled} onChange={enabled => {
                            const now = Date.now();
                            setHistoryEnabled(enabled);
                            write(K.historyEnabled, enabled);
                            setHistoryClearedAt(now);
                    ...
                    <div className={`collapse-shell ${historyExpanded ? "expanded" : ""}`}>
                        <div className="collapse-inner">
                            <div className="history-section">
                                <div className="history-head-actions"><span>{historyItems.length} записей</span>
                                    <button className="outline danger" disabled={!historyItems.length} onClick={() => {
                                        if (confirm("Очистить историю просмотра? Прогресс серий сохранится.")) {
                                            const now = Date.now();
                                            setHistoryClearedAt(now);
                                            write(K.historyClearedAt, now);
                                        }
                                ...
                                <div className="history-list">{historyItems.map(item => {
                                    const anime = known(item.animeId);
                                    return <article key={`${item.animeId}:${item.season}:${item.episode}`}>
                                        <button onClick={() => {
                                            if (!anime) return;
                                            const current = progress[item.animeId];
                                            saveProgress({
                                                ...progress,
                                                [item.animeId]: {...current, season: item.season, episode: item.episode}
                                            });
                                            openAnime(anime, true);
                                        }}>{anime?.poster?.big && <img src={anime.poster.big}
                                                                       alt=""/>}<span><b>{anime?.title ?? `Аниме #${item.animeId}`}</b><ReleaseMark
                                            anime={anime}
                                            status={cardMeta[item.animeId]?.status}/><small>Сезон {item.season} · серия {item.episode} · {formatTime(item.state.position)}</small></span>
                                            <time>{new Date(item.state.updatedAt).toLocaleDateString("ru-RU", {
                                                day: "2-digit",
                                                month: "short",
                                                hour: "2-digit",
                                                minute: "2-digit"
                                            })}</time>
                                            <em>▶ Продолжить</em></button>
                                    </article>;
                                })}{!historyItems.length && <div
                                    className="empty history-empty">{historyEnabled ? "История очищена. Новые просмотры появятся здесь автоматически." : "Сохранение истории выключено. Прогресс просмотра продолжает сохраняться."}</div>}</div>
                            </div>
                        </div>
                    </div>
                </section>
        ...
        {view === "stats" && <StatisticsPage statistics={statistics} onHome={() => {
            setView("home");
            window.scrollTo({top: 0, behavior: "smooth"});
        }}/>}
        {view === "catalog" && <section className="library catalog-page" id="catalog">
            ...
            <div className="filter-panel"><select value={sort} onChange={e => setSort(e.target.value)}>
            ...
            </select><select value={groupFilter} onChange={e => setGroupFilter(e.target.value)}>
            ...
            </select><select value={formatFilter} onChange={e => setFormatFilter(e.target.value)}>
                <option value="all">Фильмы и сериалы</option>
                <option value="series">Только сериалы</option>
                <option value="movie">Только фильмы</option>
            </select><label>Год от <input type="number" value={yearFrom} onChange={e => setYearFrom(e.target.value)}
                                          placeholder="1990"/></label><label>до <input type="number" value={yearTo}
                ...
                <button className="random-trigger" onClick={() => setRandomOpen(v => !v)}>⚄ Рандом</button>
            </div>
            {randomOpen && <div className="random-panel">
                <div><label>Жанр<select value={randomGenre}
                                        onChange={e => setRandomGenre(e.target.value)}>{genres.map(g => <option
                    key={g}>{g}</option>)}</select></label><label>Год от<input type="number" value={randomYearFrom}
                                                                               onChange={e => setRandomYearFrom(e.target.value)}
                                                                               placeholder="1990"/></label><label>до<input
                    type="number" value={randomYearTo} onChange={e => setRandomYearTo(e.target.value)}
                    placeholder="2026"/></label><label>Рейтинг от<select value={randomRating}
                ...
                <button className="primary" disabled={!randomCandidates.length} onClick={() => {
                ...
                <small>{randomCandidates.length ? `${randomCandidates.length} подходящих франшиз` : "Нет аниме с такими фильтрами"}</small>
            </div>}
            <div className="genre-row">{genres.map(g => <button key={g} className={genre === g ? "selected" : ""}
                                                                onClick={() => setGenre(g)}>{g}</button>)}</div>
            {error && <div className="empty">{error}</div>}
            <div className="cards">{visible.map(a => <AnimeCard key={a.anime_id} anime={a} meta={cardMeta[a.anime_id]}
                                                                ...
                                                                progress={progress[a.anime_id]}/>)}</div>
            {!query && <button type="button" className="load-more" disabled={loading}
                               onClick={() => void loadMore()}>{loading ? "Загружаем новые аниме…" : "Показать ещё"}</button>}
        </section>}
        <footer><Brand/><span className="api-thanks">Огромная благодарность разработчикам YummyAnime за предоставленный API — только благодаря им был создан AnimeSoul.</span><span>Прогресс и настройки сохраняются на этом ПК</span><span
            className="app-version">Версия {APP_VERSION}</span></footer>
        ...
        />}
        {folderPicker &&
            <FolderPicker anime={folderPicker} folders={folders} onToggle={toggleFolder} onCreate={createFolder}
                          onClose={() => setFolderPicker(null)}/>}
        {openedFolder && <FolderView folder={folders.find(f => f.id === openedFolder.id) ?? openedFolder} known={known}
                                     progress={progress} cardMeta={cardMeta} onOpen={(a, resume) => {
            setOpenedFolder(null);
            openAnime(a, resume);
        }} onNote={(id, note) => saveFolders(folders.map(f => f.id === openedFolder.id ? {
            ...f,
            notes: {...(f.notes ?? {}), [id]: note}
        } : f))} onReorder={(from, to) => saveFolders(folders.map(f => f.id === openedFolder.id ? {
            ...f,
            animeIds: reorder(f.animeIds, from, to)
        } : f))} onDelete={() => {
            if (confirm(`Удалить папку «${openedFolder.name}»?`)) deleteFolder(openedFolder);
