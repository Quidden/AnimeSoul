...
- [`STYLES.md`](STYLES.ru.md) обновлён одновременно.
...
Frontend gate включает strict TypeScript, ESLint warning ceiling, тесты
критической логики, CSS duplicate audit, production build и бюджеты:
...
Дополнительные инварианты: fieldUpdatedAt меняется только для изменённого поля; resetAt не позволяет вернуть сброшенные серии; profile IDs объединяются. После изменения API обновите API_SCHEMA через tools/check_docs.py --write-api и обе языковые версии.
