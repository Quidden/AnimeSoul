...
Актуально для версии **0.2.7**, реализация `app/`. Документ является обзорной
...
| --- | --- |
| что и где находится | [`PROJECT_MAP.md`](PROJECT_MAP.ru.md) |
| как устроены слои и зависимости | [`../ARCHITECTURE.md`](../ARCHITECTURE.ru.md) |
| откуда начинается и где заканчивается выполнение | [`ENTRY_POINTS_AND_FLOWS.md`](ENTRY_POINTS_AND_FLOWS.ru.md) |
| какие функции вызывают друг друга | [`ENTRY_POINTS_AND_FLOWS.md`](ENTRY_POINTS_AND_FLOWS.ru.md) |
| все REST/WS поля | [`API_REFERENCE.md`](API_REFERENCE.ru.md) |
| какие поля YummyAnime реально используются | раздел upstream fields в [`API_REFERENCE.md`](API_REFERENCE.ru.md#реально-используемые-поля-yummyanime) |
| схема JSON, progress, settings и localStorage | [`DATA_MODEL.md`](DATA_MODEL.ru.md) |
| где лежат и как применяются стили | [`STYLES.md`](STYLES.ru.md) |
| как работает OAuth/Drive merge | [`GDRIVE_SYNC.md`](GDRIVE_SYNC.ru.md) |
| как переносить данные в legacy и обратно | [`../SAVE_COMPATIBILITY.md`](../SAVE_COMPATIBILITY.ru.md) |
| что осталось рефакторить | [`REFACTORING_RECOMMENDATIONS.md`](REFACTORING_RECOMMENDATIONS.ru.md) |
...
1. React SPA показывает каталог, библиотеку, плеер, оценки, статистику и
   настройки.
2. FastAPI скрывает upstream token, объединяет каталог YummyAnime/Kodik и обслуживает
   внутренние функции.
...
[`ENTRY_POINTS_AND_FLOWS.md`](ENTRY_POINTS_AND_FLOWS.ru.md).
...
[`API_REFERENCE.md`](API_REFERENCE.ru.md).
...
| поиск | `useCatalogController` → catalog API → Yummy router → HybridCatalogueService → Yummy/Kodik → cards |
| player | card → `Watch` → family/details → videos → source/episode → direct AnimeSoulPlayer или iframe |
...
portable profile. Подробности: [`DATA_MODEL.md`](DATA_MODEL.ru.md).
...
Полная карта owners/selectors/import order: [`STYLES.md`](STYLES.ru.md).
...
Точные приоритеты, JSON-поля и runtime-файлы: [`DATA_MODEL.md`](DATA_MODEL.ru.md).
...
## Дополнения текущего кода

Сверено 16 сентября 2026 года. Отдельные подробные руководства: [UI](UI.ru.md), [Backend](BACKEND.ru.md), [46 маршрутов и модели](API_SCHEMA.ru.md), [скриншоты](SCREENSHOTS.ru.md).

Android встраивает Python/WebView, отключает Watch Party и добавляет mini-player/PiP/MediaStore/Cast. Каталог сохраняет идентичность источника; даты Jikan загружаются независимо от видео. Комнаты и download queue — только память процесса.

Переносимый snapshot дополнен fieldUpdatedAt; progress содержит resetAt. Merge объединяет профили и сравнивает отдельные поля, а не только время всего документа. Первое слияние защищено backend, cloud restore создаёт backup. Подробности в обновлённых DATA_MODEL и GDRIVE_SYNC.
