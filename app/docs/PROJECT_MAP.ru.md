...
## Дополнительные актуальные владельцы

- `app/tools/check_docs.py`: проверка двуязычных ссылок и генерация API_SCHEMA.
- `catalog.py` / `AnimeIdentityRegistry`: гибридные ответы и постоянный реестр идентичности.
- `storageSafety.ts`: версии полей, защита восстановления; `profileDocument.ts`: changedFieldRevisions при сборке снимка.
- `api/gdrive.py`: server initial-choice guard и Android complete-auth; `storage.py`: общие locks и backup.
- Детальные карты UI/сервисов: [UI](UI.ru.md), [BACKEND](BACKEND.ru.md).
