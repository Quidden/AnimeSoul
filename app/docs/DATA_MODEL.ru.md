...
имя `Своя`. Применение к CSS описано в [`STYLES.md`](STYLES.ru.md).
...
[`../SAVE_COMPATIBILITY.md`](../SAVE_COMPATIBILITY.ru.md), cloud merge — в
[`GDRIVE_SYNC.md`](GDRIVE_SYNC.ru.md).
...
## Независимые версии полей и сброс прогресса

`buildProfileSnapshot → changedFieldRevisions` сохраняет `fieldUpdatedAt` для favorites/folders/progress/ratings/tracked/theme/toolbar/playerPrefs/history/layout. Метка обновляется только при изменении самого поля. Backend нормализует JS миллисекунды и Unix секунды; для старого документа без метки использует updatedAt документа. Поэтому позднее сохранение прогресса на старом устройстве не откатывает новую тему.

`AnimeProgress.resetAt` — tombstone сброса. При merge берётся максимум resetAt обеих сторон; серии с updatedAt <= resetAt отбрасываются, более поздние остаются. Профили объединяются по ID: отдельной tombstone удаления профиля пока нет. [Точная политика](GDRIVE_SYNC.ru.md).

Дополнительные файлы устройства: `api-credentials.json` (Yummy), `animesoul-offline-settings.json`, `animesoul-kodik-private.dpapi`, `.animesoul-library.json` в каталоге загрузок, `animesoul-anime-identities.json`, `gdrive-pending-oauth.json`. Они, скачанные медиа и очередь не являются portable profile. Android держит приватные данные в sandbox, готовые MP4 — в MediaStore Movies/AnimeSoul.

Сохранённый через UI Yummy token имеет приоритет над базовым environment/config токеном в router. Сохранённые Drive credentials имеют приоритет над environment/config fallback. `ANIMESOUL_MOBILE=android` управляет backend-платформой, `VITE_ANIMESOUL_PLATFORM=android` — отдельной frontend-сборкой dist-android.

JsonStorage требует непустой profiles array, строковые непустые IDs и object snapshots; заданный activeProfile обязан существовать. Lock общий для одного пути, cloud replacement создаёт backup. 404 отличается от повреждения/ошибки чтения; последнее не разрешает bootstrap поверх файла.
