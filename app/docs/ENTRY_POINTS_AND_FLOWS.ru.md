...
4. Подключаются routers Yummy, Kodik, downloads, episode_dates, storage, party, gdrive, ratings; party исключён на Android.
5. Регистрируется `/api/health`.
6. Если `frontend/dist` существует, монтируются `/assets` и SPA catch-all.

HTTP/WS точки перечислены в [`API_REFERENCE.md`](API_REFERENCE.ru.md).
...
Точная функция выбора готовности — `wait_until_ready`, проверяющая `/api/health`. Повторное использование требует совместимых capabilities, включая kodik-direct-stream-v1, помимо ok/stack.
...
-> HybridCatalogueService.catalogue -> YummyAnimeGateway.search / KodikAnimeGateway
...
-> Player.fetchVideos
   ...
   -> dedup по video_id
-> PlayerToolbar + SeasonList + AnimeSoulPlayer или provider iframe
```
...
```text
useHeaderCloudSync effect (Drive connected)
...
CSS cascade и inline surfaces подробно описаны в [`STYLES.md`](STYLES.ru.md).


## Цепочка 16: прямой поток, загрузки и Cast

```text
Watch → fetchKodikStream → POST /api/kodik/stream
 → OfflineLibraryService.playback_source → KodikSourceResolver.resolve_playback_api
 → AnimeSoulPlayer → HLS/video events → progress

DownloadPicker → checkDownloadAvailability → POST /api/downloads/availability
 → enqueueDownload → POST /api/downloads/jobs → OfflineLibraryService.enqueue
 → sequential worker → resolver → download/FFmpeg → files/index
 → jobs/library polling → DownloadsPage → useOfflinePlayback → media endpoint

Android: AnimeSoulDownloads → DownloadForegroundService / DownloadNetworkMonitor
 → POST network → pause/resume; NativeDownloadSupport → FFmpegKit/MediaStore

useAndroidCast → castMediaSource → sendCastCommand → AnimeSoulCast.postMessage
 → CastController → Google receiver → cast-state → player/session bar/progress
```

Очередь живёт в памяти; completed media/index сохраняются. Cast принимает только прямые онлайн HTTPS потоки, backend не открывается в LAN. Возврат на телефон оставляет видео на паузе.

## Цепочка 17: безопасная запись и Android OAuth

`buildProfileSnapshot → changedFieldRevisions` обновляет только изменённые поля. `resetAt` отсекает старые серии при merge. Ошибка сохранения повторяется autosave с задержками 0/500/1500 мс, с AbortController для отмены устаревшей операции. Только 404 разрешает bootstrap; ошибки файла не перезаписываются зеркалом.

`PUT storage` проверяет choice_pending на сервере. Explicit sync требует resolve_initial_choice для первого выбора. Cloud restore использует replace_with_backup. Shared path lock защищает запись разных объектов JsonStorage.

Android callback → save_pending_oauth → foreground /complete-auth → lock → exchange_code ровно один раз → status/first choice. Disconnect пытается revoke у Google, затем очищает tokens/pending. [Правила Drive](GDRIVE_SYNC.ru.md), [UI](UI.ru.md), [Backend](BACKEND.ru.md).
