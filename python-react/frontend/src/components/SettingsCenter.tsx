      ...
      <section ref={modalRef} className="settings-modal" role="dialog" aria-modal="true" aria-label="Настройки AnimeSoul">
        ...
        <div className="settings-scroll">
          ...
          <section className="settings-group"><div className="settings-group-title"><b>Совместный просмотр</b><span>Комнаты через Hamachi, Tailscale или домашнюю сеть</span></div>
            ...
            <Setting title="Адрес комнаты" description="Пустое поле использует текущий FastAPI-сервер. Участник вводит IP компьютера хоста в Hamachi или Tailscale и выбранный порт AnimeSoul." example="http://25.10.20.30:8000">
              <input className="settings-text-input" placeholder="Текущий сервер AnimeSoul" value={props.playerPrefs.watchPartyServer} onChange={event => setPrefs({ watchPartyServer: event.target.value })} />
            </Setting>
            <Setting title="Мой режим" description="Следовать — повторять серию, таймкод, запуск и паузу хоста. Свободный — смотреть независимо и видеть позиции остальных.">
              ...
              <div className="watch-party-guide-content">
                <section>
                  ...
                  <ol>
                    ...
                    <li>На компьютере хоста FastAPI-сервер AnimeSoul должен быть доступен на выбранном при первом запуске порту.</li>
                ...
                <section>
                  ...
                  <ol>
                    ...
                    <li>Оставь адрес комнаты пустым — будет использован текущий адрес AnimeSoul.</li>
                    ...
                    <li>Запуск, пауза, серия и перемотка хоста будут ориентиром для режима следования.</li>
                  </ol>
                </section>
                <section>
                  ...
                  <ol>
                    ...
                    <li>В поле «Адрес комнаты» укажи адрес хоста, например <code>http://25.10.20.30:8000</code>.</li>
                    <li>Открой то же аниме, включи совместный режим и введи полученный код комнаты.</li>
                    <li>Выбери «Следовать за хостом» либо «Свободный просмотр / Режим медленного интернета».</li>
                  </ol>
                </section>
                <section>
                  <h3>Как работают режимы</h3>
                  <dl>
                    <div><dt>Следовать за хостом</dt><dd>Плеер повторяет серию, запуск, паузу и таймкод хоста. Озвучка переключается согласно отдельной настройке «Озвучка в комнате».</dd></div>
                    <div><dt>Свободный просмотр</dt><dd>Ничего не перематывается принудительно. Видны серия и таймкоды остальных; кнопка «Догнать хоста» синхронизирует вручную. Этот режим удобнее при медленном интернете.</dd></div>
                ...
                <section className="watch-party-troubleshooting">
                  ...
                  <dl>
                    <div><dt>«Сервер комнаты недоступен» или Not found</dt><dd>Полностью перезапусти AnimeSoul и проверь адрес и порт в <code>animesoul.python.json</code>.</dd></div>
                    <div><dt>Друг не подключается</dt><dd>Проверь, что вы видите друг друга в Hamachi/Tailscale. Разреши Python/AnimeSoul в брандмауэре Windows для частных сетей и открой выбранный TCP-порт.</dd></div>
